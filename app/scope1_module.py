...
class HumanQueueAction(BaseModel):
    erp_id: str
    action: str
...
def seed_data(force: bool = False) -> None:
    ...
    with closing(get_conn()) as conn:
        ...
        log_event(conn, "seed_completed", "system", "seed", {"erp_transactions": len(erp_seed), "bank_transactions": len(bank_seed), "pipeline_version": PIPELINE_VERSION, "model_version": MODEL_VERSION})
        conn.commit()
...
def fetch_candidate_bank_rows(conn: sqlite3.Connection, erp_row: sqlite3.Row) -> List[sqlite3.Row]:
    ...
    return [row for row in rows if date_diff_days(erp_row["tx_date"], row["tx_date"]) <= 3]
...
def exact_match(erp: sqlite3.Row, candidates: List[sqlite3.Row]) -> Tuple[Optional[sqlite3.Row], List[Dict[str, Any]]]:
    ...
    for candidate in candidates:
        ...
        scored.append({"bank_tx_id": candidate["id"], "bank_external_id": candidate["external_id"], "amount_equal": amount_equal, "reference_equal": ref_equal, "day_diff": day_diff, "passed": passed})
        if passed:
...
def fuzzy_match(erp: sqlite3.Row, candidates: List[sqlite3.Row]) -> Tuple[Optional[sqlite3.Row], List[Dict[str, Any]]]:
    ...
    for candidate in candidates:
        ...
        scored.append({"bank_tx_id": candidate["id"], "bank_external_id": candidate["external_id"], "similarity": similarity, "amount_equal": amount_equal, "day_diff": day_diff, "passed": ok})
        if ok:
...
def ml_rank(erp: sqlite3.Row, candidates: List[sqlite3.Row]) -> Tuple[Optional[sqlite3.Row], List[Dict[str, Any]], float]:
    ...
    for candidate in candidates:
        ...
        scores.append({"candidate": candidate, "probability": probability, "features": {"amount_diff_ratio": round(features[0], 4), "day_diff": int(features[1]), "description_similarity": round(features[2] * 100, 2), "reference_match": bool(features[3]), "account_match": bool(features[4])}})
    scores.sort(key=lambda item: item["probability"], reverse=True)
    rendered = [{"bank_tx_id": item["candidate"]["id"], "bank_external_id": item["candidate"]["external_id"], "probability": round(item["probability"], 4), "features": item["features"]} for item in scores]
    top = scores[0]
    return top["candidate"], rendered, top["probability"]


def create_decision(conn: sqlite3.Connection, erp_row: sqlite3.Row, bank_row: Optional[sqlite3.Row], stage: str, status: str, confidence: float, reasoning: str, human_required: bool) -> int:
    ts = now_iso()
    cur = conn.execute(
...
def export_lake_snapshot(conn: sqlite3.Connection) -> str:
    snapshot = {"exported_at": now_iso(), "app_version": APP_VERSION, "pipeline_version": PIPELINE_VERSION, "model_version": MODEL_VERSION, "decisions": [to_dict(row) for row in conn.execute("SELECT * FROM decisions ORDER BY id").fetchall()], "events": [{**to_dict(row), "payload_json": json.loads(row["payload_json"])} for row in conn.execute("SELECT * FROM events ORDER BY id").fetchall()]}
...
def reconcile_once() -> Dict[str, Any]:
    with closing(get_conn()) as conn:
        ...
        log_event(conn, "batch_started", "batch", batch_id, {"app_version": APP_VERSION, "pipeline_version": PIPELINE_VERSION, "model_version": MODEL_VERSION, "thresholds": THRESHOLDS.__dict__})
        ...
        for erp in unprocessed:
            ...
            log_event(conn, "candidates_generated", "erp_transaction", entity_id, {"erp_transaction": to_dict(erp), "candidate_count": len(candidates), "candidate_external_ids": [row["external_id"] for row in candidates]})
            exact_candidate, exact_scores = exact_match(erp, candidates)
            log_event(conn, "exact_matching_evaluated", "erp_transaction", entity_id, {"scores": exact_scores})
            if exact_candidate is not None:
                ...
                log_event(conn, "decision_created", "decision", str(decision_id), {"erp_external_id": erp["external_id"], "bank_external_id": exact_candidate["external_id"], "stage": "exact", "status": "auto_approved", "confidence": 0.99, "reasoning": reason})
            ...
            log_event(conn, "fuzzy_matching_evaluated", "erp_transaction", entity_id, {"scores": fuzzy_scores, "similarity_threshold": THRESHOLDS.fuzzy_similarity})
            if fuzzy_candidate is not None:
                ...
                if erp["amount"] >= THRESHOLDS.materiality:
                    ...
                    log_event(conn, "decision_created", "decision", str(decision_id), {"erp_external_id": erp["external_id"], "bank_external_id": fuzzy_candidate["external_id"], "stage": "fuzzy", "status": "human_review", "confidence": round(similarity / 100.0, 4), "reasoning": reason, "reason": "materiality_threshold"})
                ...
                log_event(conn, "decision_created", "decision", str(decision_id), {"erp_external_id": erp["external_id"], "bank_external_id": fuzzy_candidate["external_id"], "stage": "fuzzy", "status": "auto_approved", "confidence": round(similarity / 100.0, 4), "reasoning": reason})
            ...
            log_event(conn, "ml_matching_evaluated", "erp_transaction", entity_id, {"scores": ml_scores, "thresholds": {"auto_approve_ml": THRESHOLDS.auto_approve_ml, "review_ml": THRESHOLDS.review_ml, "materiality": THRESHOLDS.materiality}})
            if ml_candidate is not None and ml_probability >= THRESHOLDS.auto_approve_ml and erp["amount"] < THRESHOLDS.materiality:
                ...
                log_event(conn, "decision_created", "decision", str(decision_id), {"erp_external_id": erp["external_id"], "bank_external_id": ml_candidate["external_id"], "stage": "ml", "status": "auto_approved", "confidence": round(ml_probability, 4), "reasoning": reason})
            ...
            if ml_candidate is not None and ml_probability >= THRESHOLDS.review_ml:
                ...
                log_event(conn, "decision_created", "decision", str(decision_id), {"erp_external_id": erp["external_id"], "bank_external_id": ml_candidate["external_id"], "stage": "ml", "status": "human_review", "confidence": round(ml_probability, 4), "reasoning": reason})
            ...
            log_event(conn, "decision_created", "decision", str(decision_id), {"erp_external_id": erp["external_id"], "bank_external_id": None, "stage": "human", "status": "human_review", "confidence": round(max(ml_probability, 0.0), 4), "reasoning": reason})
            results.append({"erp": erp["external_id"], "stage": "human", "status": "human_review"})
        lake_path = export_lake_snapshot(conn)
        log_event(conn, "batch_finished", "batch", batch_id, {"processed_transactions": len(unprocessed), "result_summary": results, "lake_snapshot": lake_path})
        conn.commit()
...
def compute_quality() -> Dict[str, Any]:
    ...
    total = len(decisions)
    if total == 0:
        return {"total_decisions": 0, "precision": None, "recall": None, "f1": None, "human_queue_rate": None, "auto_approval_rate": None, "avg_confidence": None}
    ...
    for d in decisions:
        ...
        if expected_stage == got_stage:
            correct_stage += 1
        if expected_bank:
            if got_bank == expected_bank: true_positive += 1
            elif got_bank and got_bank != expected_bank: false_positive += 1
            elif got_bank is None: false_negative += 1
    ...
    return {"total_decisions": total, "auto_approved": len(auto_decisions), "human_review": len(human_queue), "precision": round(precision, 4), "recall": round(recall, 4), "f1": round(f1, 4), "human_queue_rate": round(len(human_queue) / total, 4), "auto_approval_rate": round(len(auto_decisions) / total, 4), "avg_confidence": round(avg_confidence, 4), "stage_accuracy": round(correct_stage / total, 4), "criteria": {"precision_goal": ">= 0.90", "human_queue_goal": "<= 0.40", "stage_accuracy_goal": ">= 0.80"}}
...
def compute_degradation() -> Dict[str, Any]:
    ...
    return {"status": status, "issues": issues, "baseline": {"precision_floor": THRESHOLDS.degradation_precision_floor, "human_queue_ceiling": THRESHOLDS.degradation_human_queue_ceiling}, "current": {"precision": quality.get("precision"), "human_queue_rate": quality.get("human_queue_rate")}, "recommended_action": "Keep current autonomy." if status == "healthy" else "Reduce auto-approval, increase human sampling, and recalibrate thresholds."}


def get_events(limit: int = 100) -> List[Dict[str, Any]]:
    with closing(get_conn()) as conn:
        rows = conn.execute("SELECT * FROM events ORDER BY id DESC LIMIT ?", (limit,)).fetchall()
        return [{**to_dict(row), "payload_json": json.loads(row["payload_json"])} for row in rows]
...
def get_pending_review() -> List[Dict[str, Any]]:
    with closing(get_conn()) as conn:
        rows = conn.execute("SELECT d.*, e.external_id AS erp_external_id, e.description AS erp_description, b.external_id AS bank_external_id, b.description AS bank_description FROM decisions d JOIN erp_transactions e ON e.id = d.erp_tx_id LEFT JOIN bank_transactions b ON b.id = d.bank_tx_id WHERE d.status = 'human_review' ORDER BY d.id").fetchall()
    return [to_dict(row) for row in rows]


def review_decision(decision_id: int, payload: ReviewPayload) -> Dict[str, Any]:
    ...
    with closing(get_conn()) as conn:
        ...
        conn.execute("UPDATE decisions SET status = ?, bank_tx_id = ?, reasoning = ?, updated_at = ?, human_required = 0 WHERE id = ?", (status, bank_tx_id, reasoning, ts, decision_id))
        if payload.action == "approve" and bank_tx_id:
            conn.execute("UPDATE bank_transactions SET matched = 1 WHERE id = ?", (bank_tx_id,))
        log_event(conn, "human_review_completed", "decision", str(decision_id), {"action": payload.action, "selected_bank_tx_id": bank_tx_id, "comment": payload.comment})
        conn.commit()
    return {"message": f"Decisão {decision_id} atualizada com {status}."}


def review_by_erp(payload: HumanQueueAction) -> Dict[str, Any]:
    pending = get_pending_review()
    target = next((item for item in pending if item["erp_external_id"] == payload.erp_id), None)
    if not target:
        raise HTTPException(status_code=404, detail="Caso não encontrado na fila humana.")
    mapped_action = "approve" if payload.action in {"confirm_match", "approve"} else "reject"
    return review_decision(int(target["id"]), ReviewPayload(action=mapped_action, selected_bank_tx_id=payload.selected_bank_tx_id, comment=f"{payload.reviewer}: {payload.note}"))


def init_scope1() -> None:
    global ML_MODEL
    create_schema()
    seed_data()
    if ML_MODEL is None:
...
def api_review_pending() -> Dict[str, Any]:
    return {"pending_review": get_pending_review()}


@router.get("/human-queue")
def api_human_queue() -> Dict[str, Any]:
    return {"cases": get_pending_review()}


@router.post("/review/{decision_id}")
def api_review_decision(decision_id: int, payload: ReviewPayload) -> Dict[str, Any]:
    return review_decision(decision_id, payload)


@router.post("/human-review")
def api_human_review(payload: HumanQueueAction) -> Dict[str, Any]:
    return review_by_erp(payload)
...
def api_health() -> Dict[str, Any]:
    return {"status": "ok", "app_version": APP_VERSION, "pipeline_version": PIPELINE_VERSION, "model_version": MODEL_VERSION, "database": str(DB_PATH)}
